...
export async function registerExistingPesertaAction(
  ...
  const updatePayload: Record<string, unknown> = {
    nominal_donasi, metode_bayar, wa_status: "pending",
  ...
  await admin.from("log_aktivitas").insert({
    ...
    record_id: pesertaId,
    detail: { nama: peserta.nama, kupons: kuponList, nominal_donasi, metode_bayar },
  });
  ...
  after(async () => {
    await sendPesertaWa({
      pesertaId,
      noWhatsapp: peserta.no_whatsapp,
      template: {
  ...
  return { success: `Peserta "${peserta.nama}" terdaftar: ${kuponList.length} kupon, ${metode_bayar.toUpperCase()}. WA dikirim di latar belakang.` };
}
